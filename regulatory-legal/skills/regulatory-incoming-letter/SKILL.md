...
description: 处理监管下发函件（行政处罚通知 / 责令整改 / 监管谈话 / 程序性通知 / 专项自查工作函 / 监管问询）的全流程：录入 + 6 分法自动归类 + 任务派发 + 进度跟踪 + 上报材料汇总。中国法本土化版的核心亮点——原版 regulatory-legal 没有这条主线。用户说"我们收到 [监管单位] 一份 [函件类型]"、"监管下发了"、"工作函"、"自查通知"、"约谈"、"监管问询"、"警示函"、"行政处罚决定书"，或上传函件 PDF / 扫描件时使用。
argument-hint: "[函件文件路径 或 粘贴函件文本]"
---

<!--
中国法新增技能，无上游英文版本。
本技能为 claude-for-legal-cn/regulatory-legal 中国法改造版专有，
处理中国监管语境下企业收到监管来函的全流程响应。

参考实现：agent-for-legal-cn/legal-skillpack-regulatory-legal/skills/regulatory-incoming-letter/
-->


# /regulatory-incoming-letter

1. 读 `~/.claude/plugins/config/claude-for-legal/regulatory-legal/CLAUDE.md` → 公司画像、escalation_path、归口部门联系人
2. 用下面的 7 阶段工作流
...
**事项上下文**：核查实务层级 `CLAUDE.md` 中 `## 事项工作区`。如果 `已启用` 是 `✗`（企业内部法务用户默认），跳过本段——技能用实务层级上下文，事项机制不可见。如果启用且没有当前事项，问："这是哪个事项？跑 `regulatory-matter-workspace switch <slug>` 或说切到实务层级。"
...
中国企业内部法务最高频的工作是**响应**监管下发的函件——这与原版的"主动监控"工作流方向相反，是中国法本土化版的**核心增量**。本技能处理：
...
- **闭环**：等监管回函（通过 `reg-feed-watcher` 或人工录入），决定是否结案

**与 `/regulatory-legal:reg-feed-watcher` 的区别**：

- reg-feed-watcher：**主动监控**——抓取监管公开发布的规则、处罚通报、答记者问等
- 监管来函处置：**被动响应**——处理监管**直接发给本公司**的函件（有明确受文单位 = 本公司）

reg-feed-watcher 可能会捕获到"金管总局自查工作函"等普遍性通知，**如本公司也是受文单位**，应转入本技能流程做完整处置。
...
| `函件类型 (初判)` | 否 | 大模型的初步判断，**第 2 步会用 6 分法重新核定** |
| `受文单位` ✱ | 是 | 必须是本公司全称（如果不是→函件转错，确认后退回） |
| `抄送范围` | 否 | 其他抄送方（如"抄送：上海地方金融监管局"） |
| `要求事项` ✱ | 是 | **列表**，从函件正文抽取每条具体要求 |
...
抽取后落地为结构化 YAML 文件到 `~/.claude/plugins/config/claude-for-legal/regulatory-legal/incoming-letters/<letter-id>/letter.yaml`，附原始扫描件 / 文本副本到同目录。
...
1. **时效核查**：多数监管措施类决定也可申请行政复议/诉讼，时效与处罚相同（60 日/6 月）
2. **限期整改→拆解为 `inspection-task`**
3. **整改报告（如要求书面回复）**：收文 + 缓冲 30/15/5 天三档预警

### 第 3 类：监管谈话类

**触发动作**：

1. **派人到场的准备**：谁去（默认派"业务对应分管 + 法务"）、准备材料
2. **谈话纪要的处置**：如要求出具书面承诺/整改计划，拆解为 `inspection-task`
3. **是否触发行政复议/诉讼**：一般认为单独的监管谈话不可诉，但约谈通报如含"责令整改"等内容，按第 2 类处理
...
3. **`escalation_path.重大业务影响决策` 紧急触发**：是否申请听证？是否提交陈述申辩？谁去？
...
1. **要求事项拆解为 `inspection-task`**：每条自查事项→一个 `inspection-task`，自动匹配归口部门
2. **进度跟踪**：30 / 15 / 5 天三档预警
3. **上报材料汇总**（到期前 X 天）：收集各部门材料→按监管要求格式生成上报草稿→会签→提交

### 第 6 类：监管问询类

**触发动作**：

1. **判断问询性质**：普通问询函 / 关注函 / 信息核实函
2. **回函起草**：按监管问询事项逐条回应；涉具体客户/业务数据需脱敏处理；**不要**主动披露问询范围外的事项
3. **是否触发后续监管动作**：问询本身一般不直接产生权利义务，**但回函内容会被监管援引**

---

## 第 4 步：任务派发（拆解为 inspection-task）

将函件要求事项拆解为 N 条 `inspection-task` 类型的整改事项，写入 `~/.claude/plugins/config/claude-for-legal/regulatory-legal/gap-tracker.yaml`：
...
派发后**逐条预审**给法务联系人审核后才发 DM（继承 `gap-surfacer` 的逐条预审 + 显式批准规则）。

---

## 第 5 步：进度跟踪

复用 `gap-surfacer` 的提醒机制，但 **`inspection-task` 用三档预警**（比常规内规差异更紧）：
...
监管截止日前 **5 个工作日**（默认，可配置）自动汇总：

### 6.1 收集

遍历该 `inspection_letter_id` 下的所有 `inspection-task`，收集每条任务的 `submission_content`、`submission_received_date`，标识缺失项。
...
按 `escalation_path`：法务负责人初审 → 首席合规官（CCO）复审 → 总经理/法定代表人签字（如要求公章 + 法人签字）→ 法务联系人提交
...
- 提交后通常 7-30 天内收到回函
- 通过 `reg-feed-watcher` 监控相关监管栏目，或通过邮箱/收发室人工录入回函
- 回函如要求"补充材料"或"再次自查"→进入新一轮流程（同一 `letter-id` 下加 `round: 2`）
- 回函如确认"通过验收/不再追究"→标 `letter.yaml` 的 `status: closed` + 同步关闭关联的 `inspection-task`

**长期未收到回函**（>60 天）：标 `status: 待回函`，不自动关闭，但移出活跃台账显示。
...
```markdown
[工作产品标题——按 ## 谁在使用这个 + 角色选定，通常为"内部合规分析——公司内部使用"]

> **⚠️ 复核提示**
> - **来源**：函件 [letter-id]（原件保存在 incoming-letters/[letter-id]/）
> - **分类**：第 X 类 [类别名]——[简短依据]
...
5. **生成上报材料草稿**——各部门反馈到位后，我按模板格式生成草稿
6. **其他**——告诉我

**清单之外我会问的一个问题**：[本函件中可能被遗漏但有影响的事项——如"本次自查范围是否会牵出关联问题"、"抄送方是否会单独跟进"等]
```
...
| 上游/下游 | 协作方式 |
|---|---|
| **`reg-feed-watcher`** ← | 如果监管信息源中识别到"针对本公司"的工作函，转入本技能处理 |
| **`gap-surfacer`** → | `inspection-task` 写入 `gap-tracker.yaml`，复用其跟进/提醒/关闭机制 |
| **`policy-diff`** → | 函件中如涉及具体监管规则，可触发政策比对对照公司政策库分析 |
| **`policy-redraft`** → | 整改过程中如需修订内部政策，触发政策修订红线起草修订草案 |
| **`customize`** → | 用户可通过个性化调整：上报材料缓冲天数、字段抽取置信度阈值、各类别的默认负责人 |
...
- **不**起草处罚决定的复议申请书/诉讼起诉状（这是律师代理工作，本技能仅触发"建议启动外聘律师"的提示）
- **不**起草听证申请书/陈述申辩书（同上，建议外聘律师介入）
- **不**判定监管处罚/措施的合法性（这是法律意见，需律师出具）
- **不**自动提交上报材料（必须经法务负责人 + 首席合规官会签 + 法定代表人/法务联系人手工提交）
- **不**自动监控监管回函（与 `reg-feed-watcher` 协作 + 人工录入兜底）
- **不**做监管沟通策略建议（如"要不要主动联系执法人员"等——这是合规/公关策略，超出本技能范围）

详细 6 分法处置规则见 `references/letter-classification-rules.md`；`inspection-task` 模板见 `references/inspection-task-template.md`。
